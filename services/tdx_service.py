...
class TDXApi:
        ...
        for attempt in range(retry):
            try:
                ...
                if e.response.status_code == 429:
                    ...
                    logger.error(f"--- ❌ API 請求失敗 (HTTP Error) on URL: {url} ---", exc_info=True)
                    try:
    ...
    def _get_all_data_paginated(self, base_url: str, page_size: int = 500):
        ...
        while True:
            ...
            logger.info(f"--- 正在請求分頁資料: $top={page_size}, $skip={skip}... ---")
            page_data = self._get_api_data(paginated_url)
            
            if page_data is None:
                logger.warning(f"--- ❌ 分頁請求失敗於 $skip={skip}，將回傳目前已獲取的資料。 ---")
                break # 如果請求失敗，中斷迴圈，返回已有的資料
            
            if not page_data:
                logger.info(f"--- ✅ 已到達資料結尾，總共獲取了 {len(all_data)} 筆資料。 ---")
                # 新增調試打印：如果第一頁就是空的，可能表示 API 根本沒數據
                if not all_data and skip == 0:
                    logger.debug(f"--- Debug: First page of data for {base_url} was empty. ---")
            ...
            if len(page_data) < page_size:
                logger.info(f"--- ✅ 已獲取最後一頁資料，總共獲取了 {len(all_data)} 筆資料。 ---")
                break
